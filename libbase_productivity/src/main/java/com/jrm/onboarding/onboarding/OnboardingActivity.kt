...
class OnboardingActivity : BaseActivity<ActivityOnboardingScreenBinding>() {
    ...
    fun preloadOnboarding2() {
        preloadOb2 = true
        preloadAds(
            placementName = BaseConstants.PLACEMENT_ONBOARDING_2,
        )
    }
    ...
    public fun preloadOnboarding3() {
        ...
        if (!BaseUtils.isFinishObd()) {
            preloadAds(
                placementName = BaseConstants.PLACEMENT_ONBOARDING_3,
                onSuccess = {
    ...
    private fun preloadOnboarding4() {
        preload0b4 = true;
        preloadAds(
            placementName = BaseConstants.PLACEMENT_ONBOARDING_4,
        )
    }
    ...
    private fun preloadOnboarding6() {
        preloadOb6 = true;
        if (!BaseUtils.isFinishObd()) {
            preloadAds(placementName = BaseConstants.PLACEMENT_ONBOARDING_INTER)
        }
    }
    ...
    private fun initViewPager() {
        ...
        Logger.d("OnboardingActivity listAdsPlacement: totalPages=$totalPages, list=$listAdsPlacement")
                ...
                val dataPage: DataPage? = if (listDataPage != null && listDataPage.isNotEmpty()) {
                    if (fragmentPosition == lastFragmentPosition) {
                        ...
                        val selectedIndex = if (dataPageIndex < listDataPage.size) {
                            ...
                            val cycledIndex =
                                (dataPageIndex - listDataPage.size) % listDataPage.size
                            cycledIndex
                        }
        ...
        viewBinding.viewpagerOnboard.offscreenPageLimit = ViewPager2.OFFSCREEN_PAGE_LIMIT_DEFAULT

        viewBinding.viewpagerOnboard.isUserInputEnabled =
            !RemoteConfigManager.instance!!.boostFNativeObd
        ...
        viewBinding.viewpagerOnboard.registerOnPageChangeCallback(object :
            ...
            override fun onPageSelected(position: Int) {
                ...
                // Track screen view
                YNMAirBridgeDefaultEvent.pushEventScreenView(
    ...
    public fun onClickNext() {
        if (viewBinding.viewpagerOnboard.currentItem == viewBinding.viewpagerOnboard.adapter!!.itemCount - 1) {
            ...
            viewBinding.viewpagerOnboard.currentItem += 1
        }
    }
    ...
    private fun goToNextActivity() {
        ...
        loadAds(
            BaseConstants.PLACEMENT_ONBOARDING_INTER,
            onSuccess = { goHome() },
            onFailure = { goHome() })
    }
...
//            else ContextCompat.getColor(this, com.ads.nomyek_admob.R.color.textColor))
        viewBinding.tvNext.text =
            (if (pos == getLastPos()) resources.getString(R.string.start) else resources.getString(R.string.next))
